...
uint64_t calculate_total_size(const Replica::Descriptor& replica) {
    if (replica.is_memory_replica()) {
        return replica.get_memory_descriptor().buffer_descriptor.size_;
    }
    if (replica.is_ssd_pool_replica()) {
        const auto& ssd_desc = replica.get_ssd_extent_descriptor();
        if (ssd_desc.object_size > 0) {
            return ssd_desc.object_size;
        }
        return ssd_desc.lba_count * static_cast<uint64_t>(ssd_desc.block_size);
    }
    if (replica.is_disk_replica()) {
        return replica.get_disk_descriptor().object_size;
    }
    if (replica.is_local_disk_replica()) {
        return replica.get_local_disk_descriptor().object_size;
    }
    return 0;
}
...
int allocateSlices(std::vector<Slice>& slices,
    ...
    // For non-memory replicas, split by logical object size
